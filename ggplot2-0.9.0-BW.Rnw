...
\textbf{\large{5: scales package}}  & \textbf{\large{22}}  \\
...
\newpage
\SweaveOpts{fig.align=center,cache=TRUE,message=FALSE,highlight=FALSE,tidy=FALSE}
...
q <- ggplot(diamonds, aes(x = table, fill = clarity)) +
      geom_histogram() + scale_y_continuous(labels = comma)
...
<<guide-legend-ex2,fig.width=4,fig.height=2.5>>=
q + theme_bw() +
...
<<guide-colorbar-ex2,fig.width=4,fig.height=3.5,out.width=0.45\textwidth,fig.show=hold>>=
p1 + scale_fill_continuous(guide = "colorbar")
...
<<guide-colorbar-ex3,fig.width=4,fig.height=2.5>>=
ggplot(mtcars, aes(x = disp, y = mpg, colour = wt)) +
   geom_point() +
   scale_colour_gradient(low = 'blue', high = 'orange', guide = 'colorbar')
@
...
ggplot(mtcars, aes(x = factor(vs), y = mpg)) +
   theme_bw() +
   geom_boxplot(aes(fill = factor(vs)), outlier.color = NA) +
...
<<geom-violin-ex1,fig.width=3,fig.height=3,out.width=0.31\textwidth,fig.show=hold>>=
...
ggplot(dat, aes(x=x, y=y)) + geom_violin() + geom_point(shape=21) +
    xlab('cyl')
ggplot(dat, aes(x = x, y = y)) + theme_bw() +
...
m <- ggplot(mtcars, aes(mpg, wt, colour = factor(cyl))) +
       theme_bw() +  geom_point() + labs(colour = 'cyl') +
       scale_colour_manual(breaks = levels(factor(mtcars$cyl)),
            values = c('blue', 'deeppink', 'orange'))
...
<<facet-grid-2d-example2,fig.width=4,fig.height=3.5,out.width=0.48\textwidth,fig.show=hold>>=
m + facet_grid(vs ~ am, scales = 'free_x') +
    opts(legend.position = 'top')
# Add labeller to distinguish levels of vs from those of am:
...
Except for the \func{\_pal} function, these will normally be used
