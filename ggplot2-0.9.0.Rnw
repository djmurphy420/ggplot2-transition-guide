...
%\newpage
\SweaveOpts{fig.align=center,highlight=TRUE,cache=TRUE,message=FALSE,tidy=FALSE}
...
in the file \code{ggplot2-0.9.0.Rnw}. By default, the file is set up
for color highlighting and color printing. For a B/W printer friendly
version, you need to make two changes: (i) in the first \R\ code chunk
immediately after the document begins (labeled `setup'), set
    \code{bw\_version  <- TRUE}; (ii) in the
\code{SweaveOpts{}} command just before the first section begins, set
