...
ggplot(ISsumm, aes(x = spray, y = mean)) +
    ...
    annotation_raster(hadley, xmin = 5.5, xmax = 6.5,
                              ymin = 30, ymax = 40)
@
\noindent Note that the \func{read.jpeg} function comes from the
\pkg{ReadImages} package, designed to read JPEG and PNG files.
...
# Read in the R logo:
Rlogo <- read.jpeg(file.path(R.home("doc"), "html", "logo.jpg"))
# Melt and plot:
dfm <- melt(DF, id = "x")
