...
%\setsansfont{Tahoma}
%\setmonofont[Scale=MatchLowercase,Mapping=tex-ansi]{Inconsolata-dz}
...
to the efforts of Kohske Takahashi. The two functions
...
We will show a few that may occur commonly in practice in the
...
\multirow{2}{1cm}{key options}
   ...
   & \code{direction}   & \code{"horizontal"} or \code{"vertical"} \\ \hline
  \end{tabular}
\end{center}
...
and/or labels that are covered by the \func{theme\_text} function.
...
<<guide-legend-ex2>>=
<<gl2-bw,eval=bw_version,echo=bw_version,fig.width=4.5,fig.height=2.5>>=
q + theme_bw() + scale_fill_discrete(
    ...
    xlim(45, 75)
@
<<gl2-col,eval=!bw_version,echo=!bw_version,fig.width=4.5,fig.height=2.5>>=
...
legends can be merged into one, ae we saw with the plot of \code{p1}
...
earlier drafts of this document. The document was processed using the
\pkg{knitr} package, developed by Yihui Xie
\url{http://yihui.github.com/knitr/}. Special thanks to Yihui for
...
on how to process the \code{.Rnw} file for color or black-and-white printing.
